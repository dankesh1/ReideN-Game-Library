<div align="center">

# <img src="https://i.ibb.co/tM25Z5tN/reidenlogo-1.png" width="34" height="34" valign="middle" alt="ReideN Logo"> ReideN Game Library

### Play over 178,000+ games for free on Steam with ReideN Game Library!

<p align="center">
  <img alt="Version" src="https://img.shields.io/badge/version-5.0.0-2f81f7?style=for-the-badge">
  <img alt="Platform" src="https://img.shields.io/badge/platform-Windows-1f6feb?style=for-the-badge">
  <img alt="Status" src="https://img.shields.io/badge/status-active-238636?style=for-the-badge">
  <img alt="Steam" src="https://img.shields.io/badge/Steam-Integrated-000000?style=for-the-badge&logo=steam">
</p>

<br>

<img width="100%" alt="ReideN Game Library Ekran Görüntüsü" src="https://i.ibb.co/8nM55HZy/mainpage.webp">

<br><br>

<p align="center">
  <a href="https://github.com/dankesh1/ReideN-Game-Library/releases">
    <img src="https://img.shields.io/badge/🚀_İndir-Releases-blue?style=for-the-badge" alt="Download">
  </a>
  <a href="https://www.virustotal.com/gui/file/0979199449c1f13cc436a689b61db0f2a08831bae575943ec40303207c09209e" target="_blank">
    <img alt="VirusTotal MSI" src="https://img.shields.io/badge/VirusTotal-MSI-blue?style=for-the-badge&logo=virustotal" />
  </a>
  <a href="https://www.virustotal.com/gui/file/bd1970d356f55017cf036663ac74ad352bfc8d9851133d9842370e63a5de9568" target="_blank">
    <img alt="VirusTotal EXE" src="https://img.shields.io/badge/VirusTotal-EXE-blue?style=for-the-badge&logo=virustotal" />
  </a>
</p>

<br>

**ReideN Game Library**, Steam'i yeniden başlatmadan oyun ekleme/kaldırma, Denuvo korumalı oyunları deneyimleme, Online Fix ile arkadaşlarınızla co-op/online oynama imkanı sunan, Rockstar, Ubisoft, EA gibi launcher doğrulamalarını bypass eden ve Steam'den kaldırılanlar dahil 178.000'den fazla oyunu tek çatı altında toplayan yeni nesil kütüphane ve oyun yönetim uygulamasıdır.

[İndir](https://github.com/dankesh1/ReideN-Game-Library/releases) · [VirusTotal (EXE)](https://www.virustotal.com/gui/file/bd1970d356f55017cf036663ac74ad352bfc8d9851133d9842370e63a5de9568) · [VirusTotal (MSI)](https://www.virustotal.com/gui/file/0979199449c1f13cc436a689b61db0f2a08831bae575943ec40303207c09209e) · [Discord](https://discord.gg/reiden) · [Hata Bildir](https://discord.gg/reiden)

</div>

---

## 📋 Genel Bakış (Overview)

**ReideN Game Library**, Steam kütüphanenizi tek bir yerden yönetmenizi sağlayan gelişmiş bir oyun yönetim uygulamasıdır. Steam'i yeniden başlatmaya gerek kalmadan oyun ekleyip kaldırabilir, Steam'den kaldırılanlar dahil 178.000'den fazla oyuna ücretsiz erişebilir, Denuvo ve Launcher Bypass (Rockstar, Ubisoft, EA) desteğiyle engellere takılmadan oynayabilir ve Online Fix ile arkadaşlarınızla co-op oyunların tadını çıkarabilirsiniz.

Ayrıca Steam ile birebir uyumlu profil özelleştirmesi sayesinde gerçek Steam puan mağazasındaki hareketli avatarları, çerçeveleri ve arka planları profilinizde ücretsiz olarak kullanabilirsiniz.

---

## ✨ Öne Çıkan Özellikler

<table>
  <tr>
    <td width="33%" align="center">
      <h3>🎮 Steam'deki 178.000+ Oyun Desteği</h3>
      <p>Steam'den kaldırılan oyunlar dahil 178.000'den fazla oyuna anında erişin ve tek tıkla kütüphanenize ekleyin.</p>
    </td>
    <td width="33%" align="center">
      <h3>⚡ Yeniden Başlatmasız Kütüphane Yönetimi</h3>
      <p>Steam istemcisini yeniden başlatmaya gerek kalmadan anında oyun ekleyin ve kaldırın.</p>
    </td>
    <td width="33%" align="center">
      <h3>🛡️ Denuvo Desteği</h3>
      <p>Denuvo korumalı oyunlarda gelişmiş kendi altyapımızla desteklenen oyunlarda güvenli ve sorunsuz deneyim. Ayrıca SteamID eşleme sayesinde kayıt (save) dosyalarınız asla bozulmaz.</p>
    </td>
  </tr>
</table>
<table>
  <tr>
    <td width="33%" align="center">
      <h3>🔓 Gelişmiş Launcher Bypass</h3>
      <p>Rockstar Games, Ubisoft Connect ve EA Play gibi harici launcher doğrulaması isteyen oyunları sorunsuz ve güvenle oynayın.</p>
    </td>
    <td width="33%" align="center">
      <h3>🌐 Online Fix & Co-op</h3>
      <p>Online Fix entegrasyonuyla arkadaşlarınızla birlikte co-op ve çok oyunculu modların tadını çıkarın.</p>
    </td>
    <td width="33%" align="center">
      <h3>🎨 Steam Birebir Profil & Puan Mağazası</h3>
      <p>Steam arkaplanları, hareketli avatarlar, çerçeveler ve başlangıç videolarıyla ücretsiz profil tasarımı.</p>
    </td>
  </tr>
</table>

---

## ⚡ Gelişmiş Altyapı ve Bağımsız Mimari

ReideN Game Library, internetteki kararsız ve güvenliği belirsiz ücretsiz üçüncü taraf araçları kullanmaz. Uygulamamız tamamen bizim tarafımızdan geliştirilen sistem ile işlemektedir:

- 🛡️ **Kendi Sistemimize Bağlı Altyapı:** Oyun ekleme/kaldırma/indirme, denuvo sistemi ve kütüphane veritabanı için kullanılan tüm DLL'ler vb. sistem bileşenleri ReideN ekibi tarafından geliştirilmektedir. SteamTools vb. uygulamalardaki gibi "Internet bağlantısı yok", "Bilinmeyen Hata" veya "Satın Al" gibi sorunlarla karşılaşmazsınız.

- 🚀 **Özel Denuvo Mimarisi & Kayıt (Save) Güvencesi:** Denuvo korumalı oyunlar için kurduğumuz sistemde paylaşımlı hesaplarla uğraşmadan oyunları doğrudan kendi şahsi Steam hesabınız üzerinden, diğer uygulamalardaki amatör manuel işlemler olmadan tek tıkla tamamen otomatik ve sorunsuz bir şekilde oynarsınız. Ayrıca Denuvo aktivasyonlarında sıklıkla karşılaşılan SteamID uyumsuzluğu ve oyun kayıtlarının (save) bozulması sorunu, akıllı SteamID eşleme altyapımızla tamamen çözülmüştür; oyun ilerlemeniz ve kayıt dosyalarınız her zaman güvende kalır.

- 🌐 **Piyasanın En Geniş Oyun Veritabanı:** Piyasada başka hiçbir yerde bulunmayan kapsamda, sürekli güncellenen ve yüksek hızlı sunucularla desteklenen **178.000'den fazla** oyunluk devasa bir veritabanına sahibiz.

- 💎 **Maksimum Kararlılık ve Kalite:** Sıradan ve kalitesiz ücretsiz sistemlerin aksine; hızlı, temiz, kesintisiz ve kurumsal standartlarda bir oyun deneyimi sunuyoruz.

---

## 🛒 Gelişmiş Mağaza Deneyimi

ReideN Game Library mağazası, oyun keşfetmeyi zahmetsiz ve pratik hale getirir:

- 🔍 **Kapsamlı Arama & Filtreleme:** Popüler yeni çıkanlar, çok satanlar ve kategorilere göre hızlı arama.
- 🏛️ **Steam'den Kaldırılan Oyunlar:** Artık Steam mağazasında satılmayan nostaljik ve efsane oyunlara anında erişim.
- 💖 **İstek Listesi & Sepet:** Beğendiğiniz oyunları istek listenize ekleyin veya sepet üzerinden toplu ekleyin.
- ⚡ **Tek Tıkla Kütüphaneye Ekleme:** Eklenen oyunlar anında kütüphanenizde kullanıma hazır hale gelir.
- 📊 **Detaylı Oyun Bilgileri:** Sistem gereksinimleri, kullanıcı yorumları, DLC listesi ve ekran görüntüleri.

---

## 📚 Kütüphane & İndirme Yönetimi

Eklediğiniz tüm içerikleri tek bir akıcı panelden kontrol edin:

* **Steam Entegre İndirme:** İndirmeleri başlatın, duraklatın ve hız sınırlaması olmadan devam ettirin.
* **Koleksiyonlar & Kategoriler:** Oyunlarınızı türlerine veya kişisel tercihlerinize göre gruplandırın.
* **DLC & Launcher Bypass Yönetimi:** Ek paketleri, Rockstar/Ubisoft/EA bypass yamalarını tek tıkla yönetin.
* **Başarım Kilidi / Yönetimi:** Steam başarımlarını görüntüleyin ve dilediğiniz gibi yönetin.
* **Manuel Oyun Ekleme:** Yerel oyun dosyalarınızı kütüphaneye kolayca bağlayın.

---

## 🎨 Steam Birebir Profil & Puan Mağazası

ReideN Game Library, Steam ile %100 birebir uyumlu ve zengin bir profil özelleştirme deneyimi sunar:

- 🖼️ **Steam Profil Arkaplanları:** Steam'deki tüm özel profil arka planlarını uygulayın.
- ✨ **Hareketli Avatarlar & Çerçeveler:** Animasyonlu avatarlar ve avatar çerçeveleriyle profilinizi canlandırın.
- 🎬 **Steam Başlangıç Videoları:** Big Picture / Steam başlangıç videolarını profilinizde ve uygulama açılışında sergileyin.
- 💎 **Ücretsiz Puan Sistemiyle Sahip Olma:** Steam puan mağazasında bulunan tüm vitrin öğelerini puan sistemiyle tamamen ücretsiz olarak elde edin ve profilinizde gururla sergileyin.
- 📊 **Oyun İstatistikleri & Vitrinler:** Başarımlarınızı, oynama saatlerinizi ve oyun geçmişinizi profilinizde paylaşın.

---

## 👥 Arkadaş Davet Sistemi (Referral)

Topluluğumuzu büyütürken ücretsiz üyelik kazanın:

- 🎁 **Kazan-Kazan Sistemi:** Özel davet kodunuzla bir arkadaşınızı davet ettiğinizde **hem davet eden siz hem de davet edilen arkadaşınız 2'şer gün ücretsiz üyelik süresi** kazanır.
- 📈 **Sınırsız Davet:** Dilediğiniz kadar arkadaşınızı davet ederek üyelik sürenizi katlayabilirsiniz.

---

## ⚙️ Ayarlar ve Kişiselleştirme

Kişisel tercihlerinizi, gizliliğinizi ve hesap güvenliğinizi kolayca yapılandırın:

* 🔒 **Hesap ve Güvenlik:** Şifre güncelleme ve profil erişim izinlerini yönetme.
* 🔗 **Entegrasyonlar:** Discord eşleştirmelerini yapılandırma.
* 👁️ **Gizlilik ve Görünürlük:** Profil görünürlüğünü, kütüphane ve aktivite paylaşım tercihlerini ayarlama.
* 🔄 **Veri ve Kütüphane Yönetimi:** Kütüphane verilerini ve oyun önbelleğini isteğe bağlı sıfırlama.
* 🎨 **Arayüz ve Tema Tercihleri:** Uygulama içi görünümü ve deneyimi kişisel zevkinize göre uyarlama.

---

## 🚀 Kurulum ve Kullanım

1. **İndirin:** [Releases](https://github.com/dankesh1/ReideN-Game-Library/releases) sayfasından en son sürümü edinin.
2. **Kurulum:** İndirilen kurulum dosyasını çalıştırın ve adımları takip edin.
3. **Giriş:** Hesabınıza giriş yapın veya saniyeler içinde yeni bir hesap oluşturun.
4. **Bağlantılar:** İsteğe bağlı olarak Steam ve Discord hesaplarınızı eşleyin.
5. **Oynamaya Başlayın:** Mağazadan dilediğiniz oyunu kütüphanenize ekleyin ve oynamaya başlayın!

> 🔒 **ReideN Game Library ticari ve tescilli bir yazılımdır.** Güvenliğiniz ve en güncel özelliklerden faydalanabilmeniz için yalnızca resmi GitHub Releases sayfamız ve resmi Discord sunucumuz üzerinden paylaşılan kurulum dosyalarını kullanınız.

---

## 📥 İndirme ve Destek

<table>
  <tr>
    <td width="33%" align="center">
      <h3>📦 İndir</h3>
      <p>En güncel yükleyiciye GitHub Releases üzerinden hemen ulaşın.</p>
      <a href="https://github.com/dankesh1/ReideN-Game-Library/releases"><b>👉 Sürümleri İndir</b></a>
    </td>
    <td width="33%" align="center">
      <h3>🛡️ VirusTotal Raporları</h3>
      <p>Şeffaf güvenlik analizi ve resmi antivirüs tarama sonuçları.</p>
      <a href="https://www.virustotal.com/gui/file/bd1970d356f55017cf036663ac74ad352bfc8d9851133d9842370e63a5de9568" target="_blank"><b>🔍 EXE Taraması</b></a> · <a href="https://www.virustotal.com/gui/file/0979199449c1f13cc436a689b61db0f2a08831bae575943ec40303207c09209e" target="_blank"><b>🔍 MSI Taraması</b></a>
    </td>
    <td width="33%" align="center">
      <h3>💬 Topluluk & Destek</h3>
      <p>Öneri, soru, destek ve hata bildirimleri için Discord sunucumuza katılabilirsiniz.</p>
      <a href="https://discord.gg/reiden"><b>👉 Discord'a Katıl</b></a>
    </td>
  </tr>
</table>

---

## ⚠️ Yasal Uyarı / Disclaimer

> Bu yazılım yalnızca eğitim, test ve kişisel arşiv yönetimi amaçlarıyla geliştirilmiştir. ReideN Game Library'nin Valve Corporation veya Steam ile doğrudan bir resmi bağı bulunmamaktadır. Tüm ticari markalar ilgili sahiplerine aittir.

---

<div align="center">

**ReideN Game Library'yi beğendiyseniz projemize sağ üstten ⭐ yıldız vererek destek olabilirsiniz!**

[ReideN-Game-Library GitHub](https://github.com/dankesh1/ReideN-Game-Library)

</div>
